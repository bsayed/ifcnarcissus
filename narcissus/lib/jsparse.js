...
Narcissus.parser = (function() {
    ...
    function FunctionDefinition(t, x, requireName, functionForm, comments) {
        ...
        // Get the IFC Security Class if any
        getSecClassIfAny(t,x,f);
        ...
        switch (tt) {
          ...
          case LEFT_BRACKET:
            ...
            while ((tt = t.peek(true)) !== RIGHT_BRACKET) {
                ...
                var elem =  AssignExpression(t, x);
                n.push(elem);

                // Update IFC security class value
                if (elem._sc !== undefined)
                    if (n._sc === undefined)
                        n._sc = elem._sc;
                    else
                        n._sc = (elem._sc > n._sc)? elem._sc : n._sc;
                ...
                do {
                    ...
                    if ((t.token.value === "get" || t.token.value === "set") &&
                        ...
                        if (t.match(COLON)) {
                            ...
                            // Update IFC security class value
                            if (n2.children[1]._sc !== undefined)
                                if (n._sc === undefined)
                                    n._sc = n2.children[1]._sc;
                                else
                                    n._sc = (n2.children[1]._sc > n._sc)? n2.children[1]._sc : n._sc;
...
*/
    ...
    /*
     * getSecClassIfAny :: (tokenizer, x, node) -> void
     *
     * This function checks if the pound sign exists.
     * If the pound sign exists, it consumes it and parses the security
     * class value after the pound symbol.
     */
    function getSecClassIfAny(t,x,n) {
        // If we have a pound sign after this primaryExp consume it.
        if (t.peek(false) === POUND) {
            t.get(true);
            //Get the security class after pound sign
            var scNode = PrimaryExpression(t,x);
            if (scNode.type !== NUMBER) {
                throw t.newSyntaxError("Wrong Security Class value, only numbers are supported. Found: " + definitions.tokens[tt]);
            } else {
                n._sc = scNode.value;
            }
        }
